...
状态：**CASE-BLIND GENERIC MODEL FROZEN**  
模型：`adult-icu-acute-hemodynamic-collapse-generic-v1`  
架构：`NCF-ARCH-1.0.0`
Runtime：`new-clinical-runtime/2.1`（final case-blind freeze）
...
模型使用 13 个可同时激活的机制过程；它们不是互斥病名，也不形成单一 simplex。
...
同一轴。每个 process 独立维护 `compensated / strained / decompensated / recovering /
failed` mode posterior。同一时刻一个 process 可 recovering，另一个可 decompensated。
...
1. 每个 process 的 local mode-weighted drift；
2. declared process-coordinate couplings；
3. declared mode couplings；
4. 已执行 action 的 active/residual/washout effect；
5. 不启动任何新 action。
