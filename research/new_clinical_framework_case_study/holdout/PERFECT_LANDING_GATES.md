...
Gate version: `NCF-PLG-1.1.0`  
Architecture: `NCF-ARCH-1.0.0`  
Machine contract: `PERFECT_LANDING_GATES.json`

This version supersedes the byte-preserved `NCF-PLG-1.0.0` artifacts.  The
only normative correction is alignment of action/counterfactual
identifiability values with the frozen architecture wire:
...
The former unqualified value `IDENTIFIED` is forbidden because it overstates
the scope of an identified claim; `OUT_OF_SCOPE` is distinct from
`UNIDENTIFIABLE`.
...
| `PL-ACT-002` | G13/G17 | counterfactual | action response updates the single state and all queries; outputs use the frozen wire enum (`IDENTIFIED_WITHIN_SCOPE`, `PARTIALLY_IDENTIFIED`, `UNIDENTIFIABLE`, `OUT_OF_SCOPE`) and include assumptions, scope and uncertainty | invalid enum or unique individual effect asserted when not identified |
