...
def _safe_run_path(repo_root: Path, relative: str, run_id: str) -> Path:
    if type(relative) is not str:
        raise ProtocolViolation("run_path must be a string")
    pure = PurePosixPath(relative)
    expected = PurePosixPath("results/unified_map/runs") / run_id
    if pure != expected or pure.is_absolute() or ".." in pure.parts:
        raise ProtocolViolation("run_path is not the canonical run location")
    root = repo_root.resolve()
    resolved = (root / Path(*pure.parts)).resolve()
    try:
        resolved.relative_to(root)
    except ValueError as exc:
        raise ProtocolViolation("run_path escapes the repository") from exc
    return resolved
...
def _verify_bound_run(
    ...
    if type(source) is not dict or set(source) - {"files", "source_digest", "reference_bundle_root", "reference_run_id"}:
    ...
    # The ordinary runner binds the ordered source-file inventory; the
    # privileged B01 upper-bound runner predates that convention and binds its
    # sole source file directly.  Both are replayable from the declared rows.
    ...
    if declared_source not in source_candidates or digests["source"] != declared_source:
...
def verify_experiment_index(
    ...
    for expected_ordinal, entry in enumerate(experiments, 1):
        _require_exact_keys(
            ...
            f"experiment entry {expected_ordinal}",
        )
