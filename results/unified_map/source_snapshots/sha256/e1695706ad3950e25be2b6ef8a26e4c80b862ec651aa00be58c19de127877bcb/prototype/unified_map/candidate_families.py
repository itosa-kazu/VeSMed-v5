...
class OpenWorldPathCausalCandidate(PathCausalStateCandidate):
    ...
    def _combined(path: np.ndarray, data: dict[str, Any]) -> np.ndarray:
        ...
        cluster_diagnosis = np.asarray(data["cluster_diagnosis"], dtype=np.float64)
        diagnosis = clusters @ cluster_diagnosis
        support = _support_transform(path, data["support"])[-2:]
        return np.concatenate((clusters, diagnosis, support))
...
class StructuralEnsembleCandidate(OpenWorldPathCausalCandidate):
    ...
    def _ensemble(path: np.ndarray, data: dict[str, Any]) -> np.ndarray:
        ...
        diagnosis = clusters @ np.asarray(causal["cluster_diagnosis"], dtype=np.float64)
        support = _support_transform(path, causal["support"])[-2:]
        return np.concatenate((clusters, lift, diagnosis, support))
