...
> **冻结日期**：2026-07-13。  
> **范围**：从 `research_notes/01_first_principles.md` 至 `08_graph_world.md` 中保留对最终架构论证最有区分力的 60 组已核实核心来源；相同论文/规范跨笔记出现时只保留一个统一 ID。另列 3 条 `BIB-ONLY` 线索，不进入证据链。  
> **重要边界**：这里的“直接核验”表示打开原出版物、规范、官方摘要或权威出版页面，不表示 60 组全部都是原始实证研究。下列来源只证明某种形式工具、数据标准、经验现象或验证框架具有所列的**局部能力或限制**；它们不共同证明某个临床核心架构已经正确，更不证明临床有效性、安全性或数学意义上的全局最优。

## 0. 核验方法、状态与引用规则

### 0.1 核验方法

1. 优先打开标准组织、出版社、会议/期刊、作者或官方项目页面；有 DOI 时同时保留 DOI。
2. 核对标题、作者、年份、出版载体，并在正文、正式摘要或规范条文中定位本表所写的支持点。
3. 只把正文/规范直接支持的最窄命题写入“支持”；由多来源交叉推出的架构判断不冒充单篇原文结论。
4. 正式规范只证明其字段和语义范围；官方产品文档只证明该实现公开承诺的行为；付费规范若只核对公开摘要，不能引用未读条款。
5. 只有书目元数据而未核对正文者标为 `BIB-ONLY`，不得用于核心结论。
6. 本次合并另做静态校验：统一 ID 唯一性、Markdown 链接语法、URL scheme、表格列数和重复 URL；这不替代未来的周期性网络可达性检查。
7. 当前账本尚未为每条来源逐项保存页码/章节、`verification_scope`、文档哈希或替代存档；因此“正文已核对”仍依赖本次研究记录，不能冒充一套可独立复演的系统综述证据库。动态规范后续应补 `verified_at/version/locator/scope/stable_id`。

### 0.2 来源类型与核验状态

- `正式规范`：标准、W3C Recommendation 或互操作规范正文已核对；Candidate/Trial Use/informative/non-binding 状态必须在具体条目保留，不能统称成熟法规。
- `原论文`：原始研究或形式化论文正文已核对。
- `权威教材/综述/学位论文`：公开正文或权威出版页面已核对，并只用于领域通行形式或边界。
- `官方文档（非规范）`：官方实现/模式文档已核对，但不能提升为跨实现标准。
- `BIB-ONLY`：只核对出版元数据，不进入核心支撑链。

### 0.3 强制引用边界

- “能表示”不得写成“运行时强制”；“在某数据集有效”不得写成“临床普遍有效”。
- provenance、因果边、统计依赖、执行 trace 和审计日志互不等价。
- 逻辑开放世界、统计 open-set/OOD、开放对象宇宙互不等价。
- 若最终结论超出一条来源的“支持”列，必须由原型实验或其他独立来源补证。

## 1. 临床语义、时间、血缘、缺失与部署边界

| ID | 来源（URL/DOI） | 来源类型 | 本研究只用它支持的有限命题 | 明确不支持什么 |
|---|---|---|---|---|
| CLIN-01 | HL7 FHIR R5：[Observation definitions](https://hl7.org/fhir/R5/observation-definitions.html)、[Condition](https://hl7.org/fhir/R5/condition.html)、[ClinicalImpression](https://hl7.org/fhir/R5/clinicalimpression.html)、[Specimen definitions](https://hl7.org/fhir/R5/specimen-definitions.html)、[DiagnosticReport definitions](https://hl7.org/fhir/R5/diagnosticreport-definitions.html) | 互操作规范（R5 STU；各资源 maturity 不同） | 交换模型分别建模测量/时点判断、诊断问题和临床评估；`effective[x]`、`issued`、标本采集/接收等字段也表明临床相关时间与结果可用时间不能合并。 | 不证明 FHIR 资源就是合适的内部计算内核；Condition/ClinicalImpression 等 Trial Use 资源也不能冒充全部 Normative；不保证来源系统完整填写这些字段，也不提供潜在状态、概率推断或严格双时态回放。 |
| CLIN-02 | HL7 FHIR R5：[Provenance](https://hl7.org/fhir/R5/provenance.html) 与 [element definitions](https://hl7.org/fhir/R5/provenance-definitions.html) | 互操作规范（Trial Use） | 可显式记录 target/version、agent、activity、entity、occurred 与 recorded，以及导入、转换、修订等来源关系。 | 不证明两份证据独立或可靠；不提供推断依赖代数、撤回级联或生理因果。 |
| CLIN-03 | HL7 FHIR R5：[Workflow](https://hl7.org/fhir/R5/workflow.html) | 互操作规范（Trial Use） | definition、request 与 event 是不同工作流模式；request 表示意图且不保证动作实际发生。 | 不证明整个 FHIR workflow 足以表达复杂临床动力学、治疗效果或动作安全策略。 |
...
| CLIN-09 | W3C, [OWL 2 Web Ontology Language Primer](https://www.w3.org/TR/owl2-primer/), Recommendation suite, 2012 | 官方 informative Primer | 开放世界语义下，知识库未陈述某事实不蕴含该事实为假；类、公理和推理可在声明式本体中扩展。 | Primer 自身不是规范性条款；不提供统计 OOD 检测、概率校准、时间回放或临床观察生成模型。 |
| CLIN-10 | IMDRF, [*Software as a Medical Device: Clinical Evaluation* (N41)](https://www.imdrf.org/documents/software-medical-device-samd-clinical-evaluation) | Final technical document（non-binding principles） | valid clinical association、analytical/technical validation 与 clinical validation 是不同证据层；部署论证不能由表示或单元测试替代。 | 不是法规；不选择内部架构，也不证明任一原型已达到临床有效性或监管可接受性。 |
...
| CLIN-13 | Cheng et al., [“Blood Culture Results Before and After Antimicrobial Administration in Patients With Severe Manifestations of Sepsis: A Diagnostic Study”](https://doi.org/10.7326/M19-1696), [PubMed](https://pubmed.ncbi.nlm.nih.gov/31525774/), *Annals of Internal Medicine* 2019 | 原论文（前瞻性诊断研究） | 在该研究人群中，抗菌药给药后的血培养敏感度低于给药前，提供“干预会改变观察通道”的真实临床反例。 | 单一研究不能定义所有病原体、药物和采样场景的通用观察模型，也不直接识别个体治疗效应。 |
...
| ID | 来源（URL/DOI） | 来源类型 | 本研究只用它支持的有限命题 | 明确不支持什么 |
|---|---|---|---|---|
| TEMP-01 | Kowalski & Sergot, [“A Logic-based Calculus of Events”](https://doi.org/10.1007/BF03037383), 1986；[作者 PDF](https://www.doc.ic.ac.uk/~rak/papers/event%20calculus.pdf) | 原论文 | Event Calculus 以事件、时间区间/性质及启动终止关系推理变化，且事件描述可乱序吸收；对此前成立、随后不再成立的时变关系，可用新增终止事件/有效期终点表达“不再成立”。 | 这不直接支持撤销一开始就错误或来源失信的证据，也不提供 knowledge-time 修订、provenance 级联撤回、概率临床推断、双时态或 actor visibility。 |
...
## 8. 知识图谱、Transformer、RAG、世界模型与数字孪生

| ID | 来源（URL/DOI） | 来源类型 | 本研究只用它支持的有限命题 | 明确不支持什么 |
|---|---|---|---|---|
| GRAPH-01 | W3C, [*RDF 1.2 Concepts and Abstract Data Model*](https://www.w3.org/TR/2026/CR-rdf12-concepts-20260407/), Candidate Recommendation Snapshot, 2026-04-07（非 Recommendation）；Gutierrez, Hurtado & Vaisman, [“Introducing Time into RDF”](https://doi.org/10.1109/TKDE.2007.34), IEEE TKDE 2007 | W3C Candidate Recommendation Snapshot + 原论文 | RDF 抽象模型以 triples/graphs/datasets 表达声明，本身是 atemporal static snapshot；Temporal RDF 通过额外的 temporal graph、entailment 与 query semantics 才加入有效时间，证明时间不是普通 RDF 图自动拥有的语义。 | 不提供 knowledge/available time、actor visibility、临床 observation model、撤回级联、概率/因果或患者动力学；Candidate status 不代表 W3C endorsement，也不能冒充冻结 Recommendation。 |
| NEURAL-01 | Vaswani et al., [“Attention Is All You Need”](https://proceedings.neurips.cc/paper_files/paper/2017/hash/3f5ee243547dee91fbd053c1c4a845aa-Abstract.html), NeurIPS 2017 | 原论文 | Transformer 用 query/key/value attention、multi-head attention、position information、masking 和逐层变换执行序列表示与读出。 | QKV/attention weight 不是临床事实类型、因果机制、证据 lineage 或安全规则；论文不证明长上下文能阻断未来泄漏、同源增信和模型外强制分类。 |
| NEURAL-02 | Lewis et al., [“Retrieval-Augmented Generation for Knowledge-Intensive NLP Tasks”](https://proceedings.neurips.cc/paper/2020/hash/6b493230-Abstract.html), NeurIPS 2020 | 原论文 | RAG 把可训练 retriever 的非参数文档记忆与生成模型组合，让生成条件化于检索到的 passages；检索层与生成层是可分模块。 | 检索结果不是完整知识集；论文不保证召回关键禁忌、来源真实性/独立性、时间版本一致性或 fail-closed，故 top-k/RAG 不能成为安全不变量是否执行的依据。 |
| WORLD-01 | Ha & Schmidhuber, [“Recurrent World Models Facilitate Policy Evolution”](https://papers.nips.cc/paper_files/paper/2018/hash/2de5d16682c3c35007e4e92982f1a2ba-Abstract.html), NeurIPS 2018；[论文 PDF](https://papers.nips.cc/paper_files/paper/2018/file/2de5d16682c3c35007e4e92982f1a2ba-Paper.pdf) | 原论文 | VAE latent、action-conditioned MDN-RNN dynamics 与 controller 构成可 rollout 的 world model；“Cheating the World Model” 实验显示 optimizer 会利用近似模拟器错误，模型内高回报可在真实环境崩溃。 | learned latent 不是可审计事实账本；生成逼真/预测准确不证明临床 `do()`、患者级反事实、OOD coverage、证据撤回或安全策略正确。 |
| TWIN-01 | National Academies of Sciences, Engineering, and Medicine, [*Foundational Research Gaps and Future Directions for Digital Twins*](https://doi.org/10.17226/26894), 2024 | 权威共识报告 | digital twin 可被界定为计算模型与特定 physical counterpart 的动态耦合系统，通过数据更新并支持预测/反馈；报告同时把 UQ、model discrepancy、causality、identifiability 和 data/model assimilation 列为关键研究缺口。 | “digital twin”标签不规定内部状态、证据、时间、因果或撤回 calculus，也不证明任意 patient twin 已有临床可信度。 |
...
- 核心表固定为 60 组来源；未来新增来源时，应说明它改变了哪一项现有命题或证据等级，而不是只增加相似书目。
- W3C Candidate Recommendation、预印本、官方产品文档和付费标准摘要必须持续保留各自状态；后续版本变化时新增核验记录，不能静默改写旧证据。
- URL 可达性会漂移。链接失效时优先替换为同一出版物的 DOI、标准组织永久页或作者存档，并保留标题、作者、年份与原核验日期。
- 任何把本账本用于最终架构决策的文档，都应引用统一 ID 及其“明确不支持”列，避免只摘取有利能力。
